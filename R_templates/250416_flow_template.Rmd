...
check_points_within_polygon <- function(data, polygon,xcol,ycol) {
  # Convert the polygon to an sf object
  polygon_sf <- sf::st_sfc(sf::st_polygon(list(as.matrix(polygon))), crs = 3857)
  
  # Convert the data to an sf object
  data_sf <- sf::st_as_sf(data, coords = c(xcol, ycol), crs = 3857)
  
  # Check if each point is within the polygon
  within_polygon <- sf::st_within(data_sf, polygon_sf, sparse = FALSE)
  
  return(within_polygon[, 1])
}
