...
df_rt <- data_mean %>%
  filter(RT == TRUE) %>%
  select(Bio_Sample, Probe, Time_hr, FAZ3532_50uM, Virus_MOI, Ct.mean, Ct.SE)
...
# ----------------------------
# 4) Reference PROBE per BIOLOGICAL SAMPLE
...
df_merged <- df_rt %>%
  ...
  left_join(ref_probe_by_sample, by = "Bio_Sample") %>%
  # cross-join the single-row constants so they're available to every row
  mutate(join_key = 1) %>%
  left_join(ref_probe_constants %>% mutate(join_key = 1), by = "join_key") %>%
  select(-join_key)
