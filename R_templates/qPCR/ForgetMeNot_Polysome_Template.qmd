...
```{r}
df <- df_poly_abund %>%
  filter(Fraction != "Total", Probe %in% c("GAPDH_S1","ATF4_S1")) %>%
  mutate(
    # Create a new column with the desired labels using recode()
    N_ribosomes = recode(Fraction,
      "1-4"   = "0",
      "5-6"   = "1",
      "7-8"   = "2-4",
      "9-10"  = "5-7",
    ...
    # IMPORTANT: Convert it to a factor with the correct order
    N_ribosomes = factor(N_ribosomes, levels = c("0", "1", "2-4", "5-7", "8+"))
  )

p <- ggplot(df,
       aes(x = N_ribosomes, y = Rel_abund_norm, color = dKO)) +
  ...
  #scale_y_log10nice() +
  facet_grid(Probe ~ .) +
  labs(x = "# of ribosomes", y = "Fraction of Total RNA") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
p
ggsave("250911_Jayabalan_U2OS_WT_dKO_qPCR_polysome_profile.pdf", egg::set_panel_size(p, width = unit(2, "cm"), height = unit(1.5,"cm")))
```
...
# 0) Parameters
# ----------------------------
ref_probe      <- "GAPDH" # internal reference RNA to normalize totals
ref_fraction       <- "Total" # identifies which fraction to use as the reference
...
#    (a single-row tibble holding constants for all rows)
# ----------------------------
ref_probe_constants <- ref_condition_by_probe %>%
  filter(Probe == ref_probe) %>%
  transmute(
    Ct.refCond.refProbe    = Ct.refCond,
    Ct.refCond.refProbe.se = Ct.refCond.se
